...
import { Card, Divider, Button, Popover, Collapse, message, Row, Col, } from 'antd';
import moment from 'moment';
import { CloseCircleFilled, WarningFilled, WarningOutlined, CheckCircleTwoTone, CloseOutlined } from '@ant-design/icons';
...
export default function BidsValidator(props) {
                ...
                <Row gutter={[16, 24]}>
                    <Col span={6}>
                        ...
                        <ul style={{ listStyleType: 'square' }}>
                            <li>{summary.totalFiles} Files, {getFileSize(summary.size)} </li>
                            <li>{summary.subjects.length > 0 ? `${summary.subjects.length} - Subjects` : '1 - Subject'}</li>
                            <li>{summary.sessions.length > 0 ? `${summary.sessions.length} - Session` : '1 - Session'}</li>
                        </ul>
                    </Col>
                    <Col span={10}>
                        <b >Available Tasks</b>
    ...
    let summaryMessage = (
        <div>
            <span style={{ marginLeft: 20 }}> Validation in progress, this might take few minutes.</span>
        </div>
    );
        ...
        summaryMessage = (
            <div>
                <div style={{ margin: '-10px 20px 0px 20px' }}>
                    ...
                    <span style={{ color: 'orange', marginLeft: 5 }}>{warnings.length} Warnings</span>
                </div>
                <Divider style={{ marginTop: 10, marginBottom: 10 }} />
                {summaryInfo}
                {summaryInfo && <Divider style={{  margin: '10px 0' }} />}
                <b style={{ marginLeft: 20, display: 'block', fontSize: 15 }}> We found {warnings.length} Warnings in your dataset. </b>
                <span style={{ marginLeft: 20, display: 'block'}}>You are not required to fix warnings, but doing so will make your dataset more BIDS compliant.</span>
                <span style={{ marginLeft: 20, display: 'block'}}>Last verified time: {moment(basicInfo.bidsUpdatedTime).format('YYYY-MM-DD HH:mm')}</span>
            </div>
        )
    }
        ...
        summaryMessage = (
            <div>
                <div style={{ margin: '-10px 20px 0px 20px' }}>
                    ...
                    <span style={{ color: 'red', marginLeft: 5 }}>{errors.length} Errors</span>
                </div>
                <Divider style={{ marginTop: 10, marginBottom: 10 }} />
                {summaryInfo}
                {summaryInfo && <Divider style={{ marginTop: 10, marginBottom: 10 }} />}
                <b style={{ marginLeft: 20, display: 'block', fontSize: 15, mariginTop: 10 }}> We found {errors.length} Errors in your dataset.</b>
                <span style={{ marginLeft: 20, display: 'block'}}>Last validated time: {moment(basicInfo.bidsUpdatedTime).format('YYYY-MM-DD HH:mm')}</span>
            </div>
        )
    }
    ...
    if (warnings && warnings.length) {
        ...
        warnings.forEach((warning, index) => {
                    ...
                    filesPanel.push(
                        <p>
                            <p>
                                ...
                                {file.file.stats && <span style={{ float: 'right' }}>{getFileSize(file.file.stats.size)}</span>}
                            </p>
                            <p>
                                <b>Location:</b>
            ...
            warningDetails.push(
              <Panel
                showArrow={false}
                header={<b>[Code {warning.code}] {warning.key}</b>}
                extra={`${warning.files.length} ${warning.files.length > 1 ? 'files': 'file'}`}
              >
                  {filesPanel}
              </Panel>  
            );
        ...
        warningsPanel = (
            <Panel
                header={<span style={{ color: '#8B6C25' }}>view {warnings.length} warnings in {count} files</span>} 
                showArrow={false} 
    ...
    if (errors && errors.length) {
        ...
        errors.forEach((error, index) => {
            ...
            errorsDetails.push(
              <Panel
                showArrow={false}
                header={<b>[Code {error.code}] {error.key}</b>}
                extra={`${error.files.length} ${error.files.length > 1 ? 'files': 'file'}`}
              >
                  {filesPanel}
              </Panel>  
            );
        ...
        errorsPanel = (
            <Panel
                header={<span style={{ color: '#681A20' }}>view {errors.length} errors in {count} files</span>} 
                showArrow={false} 
    ...
    if (errors && errors.length > 0) {
        icons.push(<CloseCircleFilled style={{ marginRight: 10, fontSize: 18, color: 'red' }} />);
        icons.push(<WarningFilled style={{ marginRight: 10, fontSize: 18, color: 'red' }} />);
    } else {
        basicInfo.bidsResult  && icons.push(<CheckCircleTwoTone twoToneColor="#52c41a" style={{ marginRight: 10, fontSize: 18 }}/>);
    }

    if (warnings && warnings.length > 0) {
        icons.push(<WarningOutlined style={{ marginRight: 10, fontSize: 18, color: 'orange' }} />);
    }
